<x-layouts.app title="Retensi Arsip">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-bold text-[var(--color-navy-900)]">Retensi Arsip</h1>
            <p class="text-sm text-[var(--color-slate-400)] mt-0.5">Jadwal retensi sesuai Perbup No. 10/2023 & No. 63/2024</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-[var(--color-slate-200)] p-4 mb-4">
        <form method="GET" class="flex gap-2">
            <div class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari kode klasifikasi atau nama kegiatan..."
            ...
            @if(request('search'))
                <a href="{{ route('retensi.index') }}" class="px-4 py-2 rounded-xl text-sm font-medium text-[var(--color-slate-400)] hover:text-[var(--color-slate-600)]">Reset</a>
            @endif
        </form>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-[var(--color-slate-200)] overflow-hidden">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-[var(--color-slate-100)]">
                        <th class="px-5 py-3 text-left text-xs font-semibold text-[var(--color-slate-400)] uppercase tracking-wider">Kode</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-[var(--color-slate-400)] uppercase tracking-wider">Jenis Arsip</th>
                        <th class="px-5 py-3 text-center text-xs font-semibold text-[var(--color-slate-400)] uppercase tracking-wider">Aktif</th>
                        <th class="px-5 py-3 text-center text-xs font-semibold text-[var(--color-slate-400)] uppercase tracking-wider">Inaktif</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-[var(--color-slate-400)] uppercase tracking-wider">Nasib Akhir</th>
                        <th class="px-5 py-3 text-center text-xs font-semibold text-[var(--color-slate-400)] uppercase tracking-wider">Batas</th>
                    ...
                    @forelse ($retensis as $r)
                        <tr class="hover:bg-[var(--color-slate-50)] transition-colors">
                            <td class="px-5 py-3.5">
                                <span class="font-mono text-sm font-semibold text-[var(--color-blue-600)] bg-[var(--color-blue-50)] px-2 py-0.5 rounded-md">{{ $r->kode_klasifikasi }}</span>
                            </td>
                            <td class="px-5 py-3.5 text-sm text-[var(--color-navy-900)] max-w-xs">{{ $r->nama_kegiatan }}</td>
                            <td class="px-5 py-3.5 text-sm text-[var(--color-slate-500)] text-center">{{ $r->masa_aktif }} th</td>
                            <td class="px-5 py-3.5 text-sm text-[var(--color-slate-500)] text-center">{{ $r->masa_inaktif }} th</td>
                            <td class="px-5 py-3.5">
                                @switch($r->nasib_akhir_default)
                                    @case('musnah')
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[var(--color-danger)]/10 text-[var(--color-danger)]">Musnah</span>
                                        @break
                                    @case('permanen')
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[var(--color-success)]/10 text-[var(--color-success)]">Permanen</span>
                                        @break
                                    @case('dinilai_kembali')
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-[var(--color-warning)]/10 text-[var(--color-warning)]">Dinilai Kembali</span>
                                        @break
                                @endswitch
                                @if($r->keterangan_nasib_akhir)
                                    <p class="text-[11px] text-[var(--color-slate-400)] mt-0.5 leading-tight max-w-[200px]">{{ $r->keterangan_nasib_akhir }}</p>
                                @endif
                            </td>
                            <td class="px-5 py-3.5 text-sm text-[var(--color-slate-500)] text-center">{{ $r->default_batas_waktu_hari ?? '-' }} hr</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center">
                                <x-icon name="archive-box" class="w-10 h-10 text-[var(--color-slate-300)] mx-auto mb-3" />
                                <p class="text-sm text-[var(--color-slate-400)]">Belum ada data retensi arsip.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($retensis->hasPages())
            <div class="px-5 py-3 border-t border-[var(--color-slate-100)]">{{ $retensis->links() }}</div>
        @endif
    </div>
    ...
    <div x-cloak
         x-data="{
            ...
            kode_klasifikasi: '', nama_kegiatan: '',
            masa_aktif: 2, masa_inaktif: 3,
            nasib_akhir_default: 'musnah', keterangan_nasib_akhir: '',
            default_batas_waktu_hari: 14,
            reset() { this.isEdit = false; this.id = null; this.kode_klasifikasi = ''; this.nama_kegiatan = ''; this.masa_aktif = 2; this.masa_inaktif = 3; this.nasib_akhir_default = 'musnah'; this.keterangan_nasib_akhir = ''; this.default_batas_waktu_hari = 14; }
         }"
        ...
        <div x-show="open"
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 my-8" @click.stop>
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-[var(--color-navy-900)]" x-text="isEdit ? 'Edit Retensi' : 'Tambah Retensi Baru'"></h3>
                ...
                <form method="POST" :action="isEdit ? '{{ url('retensi') }}/' + id : '{{ route('retensi.store') }}'">
                    ...
                    <div class="space-y-3">
                        <div>
                            <label class="block text-sm font-medium text-[var(--color-navy-700)] mb-1.5">Kode Klasifikasi *</label>
                            ...
                            <p class="text-[11px] text-[var(--color-slate-400)] mt-1">Sesuai Perbup No. 10 Tahun 2023</p>
                            @error('kode_klasifikasi') <p class="text-xs text-[var(--color-danger)] mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-[var(--color-navy-700)] mb-1.5">Jenis Arsip / Nama Kegiatan *</label>
                            ...
                            @error('nama_kegiatan') <p class="text-xs text-[var(--color-danger)] mt-1">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-3">
                            <div>
                                <label class="block text-sm font-medium text-[var(--color-navy-700)] mb-1.5">Aktif (th) *</label>
                                <input type="number" name="masa_aktif" x-model="masa_aktif" min="0" required
                                    class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-[var(--color-slate-200)]
                                           focus:outline-none focus:border-[var(--color-blue-500)] focus:ring-2 focus:ring-[var(--color-blue-500)]/10 transition-all" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-[var(--color-navy-700)] mb-1.5">Inaktif (th) *</label>
                                <input type="number" name="masa_inaktif" x-model="masa_inaktif" min="0" required
                                    class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-[var(--color-slate-200)]
                                           focus:outline-none focus:border-[var(--color-blue-500)] focus:ring-2 focus:ring-[var(--color-blue-500)]/10 transition-all" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-[var(--color-navy-700)] mb-1.5">Batas (hari)</label>
                                <input type="number" name="default_batas_waktu_hari" x-model="default_batas_waktu_hari" min="1"
                                    class="w-full px-3.5 py-2.5 text-sm rounded-xl border border-[var(--color-slate-200)]
                                           focus:outline-none focus:border-[var(--color-blue-500)] focus:ring-2 focus:ring-[var(--color-blue-500)]/10 transition-all" />
                            </div>
                        </div>
                        ...
                        <div x-show="nasib_akhir_default === 'dinilai_kembali'" x-transition x-cloak>
                            <label class="block text-sm font-medium text-[var(--color-navy-700)] mb-1.5">Keterangan Nasib Akhir</label>
                            ...
                            <p class="text-[11px] text-[var(--color-slate-400)] mt-1">Verbatim dari Perbup No. 63/2024</p>
                        </div>
                    </div>
    ...
    @if($errors->any())
        <script>document.addEventListener('alpine:init', () => setTimeout(() => window.dispatchEvent(new CustomEvent('open-retensi-modal')), 100));</script>
    @endif
</x-layouts.app>
